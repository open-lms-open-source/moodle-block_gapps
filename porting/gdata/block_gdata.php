 ...
 **/
    ...
    /**
     * User to clean admins from the sync table
     * if they are already in sync.
     * This function is meant to be run manually and so
     * it will only be run a few times.
     **/
    function clean_admins_from_sync() {
            $users = get_records('block_gdata_gapps');
            foreach($users as $user) {
                    if (is_siteadmin($user->userid)) {
                            delete_records('block_gdata_gapps','userid',$user->userid);
                    }
            }
    }

    /**
     * Hook to simply clean admins and redirect back
     */
    function cleanadmins_process() {
        global $CFG;
        $this->clean_admins_from_sync();
        $return = required_param('return',PARAM_URL);
        redirect($return);
    }

    /**
     * Hook defined so that process will get called
     */
    function cleanadmins_display() {
